...
class NucleicAcid(BioMolecule):
    ...
    def get_p_to_p_distance(self, residue_id_01, residue_id_02, periodic):
        ...
        self.get_distance(*atom_positions, store_result=True, opt=True, periodic=periodic)

    # TODO: implement multi-thread
    def get_all_p_to_p_distances(self, periodic):
        for residue_01, residue_02 in combinations(self.residues, 2):
            ...
            if 'P' in residue_01.sequence and 'P' in residue_02.sequence:
                self.get_p_to_p_distance(residue_id_01=residue_01.subsystem_index,
                                         residue_id_02=residue_02.subsystem_index, periodic=periodic)
    ...
    def get_dihedral(self, dihedral_name, residue_id, periodic):
        ...
        dihedral_angles = super(NucleicAcid, self).get_dihedral(*atom_positions, store_result=False, opt=True, periodic=periodic)
    ...
    # TODO: implement multi-thread
    def get_all_dihedral_angles(self, periodic):
        for residue in self.residues:
            for key in self.dictionary.dihedral_angles_dictionary.keys():
                try:
                    ...
                    self.get_dihedral(dihedral_name=dihedral_name, residue_id=residue.subsystem_index, periodic=periodic)
