...
class TestFourAtomsPlusStandardMethods(FourAtomsPlusMoleculeTestCase, TestThreeAtomsStandardMethods):
    def test_init(self):
        ...
        self.assertIsNone(self.molecule.angle_cutoff)
        self.assertIsNone(self.molecule.distance_cutoff)
        self.assertIsNone(self.molecule._hbonds)
...
class TestCalculateHydrogenBondMethods(FourAtomsPlusMoleculeTestCase):
    def test_calculate_bonds_attribute_change(self):
    ...
    def test_calculate_hydrogen_bonds_bonds(self):
        self.molecule.calculate_hydrogen_bonds(distance_cutoff=0.25, angle_cutoff=2.0)
...
class TestHydrogenBondMethodsCalculated(FourAtomsPlusMoleculeTestCase):
    def setUp(self) -> None:
        super(TestHydrogenBondMethodsCalculated, self).setUp()
...
class TestFourAtomsPlusStandardMethodExceptions(FourAtomsPlusMoleculeExceptionsTestCase,
    ...
    def test_init_residues_wrong_data_type(self):
        with self.assertRaises(self.exception) as context:
            self.molecule(residues=4.2, molecule_name=self.molecule_name, box_information=self.box_information, periodic=True)
